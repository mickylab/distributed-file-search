...
public class UserSearchHandler implements OnRequestCallback {
    ...
    private List<FrontendSearchResponse.SearchResultInfo> filterResults(
            SearchModel.Response searchClusterResponse, long maxNumberOfResults, double minScore) {
        ...
        for (int i = 0; i < searchClusterResponse.getRelevantDocumentsCount() && i < maxNumberOfResults; i++) {
            int normalizedDocumentScore = normalizeScore(searchClusterResponse.getRelevantDocuments(i).getScore(), maxScore);
            if (normalizedDocumentScore < minScore) continue;
    ...
    private static double getMaxScore(SearchModel.Response searchClusterResponse) {
        if (searchClusterResponse.getRelevantDocumentsCount() == 0) return 0;
    ...
    private static int normalizeScore(double inputScore, double maxScore) {
        return (int) Math.ceil(inputScore * 100 / maxScore);
    }
