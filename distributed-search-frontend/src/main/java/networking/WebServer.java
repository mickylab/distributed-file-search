...
public class WebServer {
    ...
    private void handleRequestForAsset(HttpExchange exchange) throws IOException{
        ...
        if (asset.equals(HOME_PAGE_ENDPOINT)) response = readUiAsset(HOME_PAGE_UI_ASSETS_BASE_DIR + "index.html");
        else response = readUiAsset(asset);
    ...
    private byte[] readUiAsset(String asset) throws IOException {
        ...
        if (assetStream == null) return new byte[]{};
    ...
    private static void addContentType(String asset, HttpExchange exchange) {
        ...
        else if (asset.endsWith("css")) contentType = "text/css";
    ...
    private void sendResponse(byte[] responseMessageBytes, HttpExchange exchange) throws IOException {
